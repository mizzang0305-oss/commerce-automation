...
export function CommandButton({ queueId = "", command, requestValue = "", expectedRevision, namespace = "", children, tone = "default" }: {
  queueId?: string; command: AllowedCommand; requestValue?: string; expectedRevision?: number | null; namespace?: string; children: React.ReactNode; tone?: "default" | "success" | "danger";
}) {
  ...
  async function run() {
    ...
    try {
      const response = await fetch("/api/commerce-control/commands", {
        ...
        body: JSON.stringify({ queueId, command, requestValue, expectedRevision, namespace, webRequestKey: crypto.randomUUID() })
      });

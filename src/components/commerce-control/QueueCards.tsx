    ...
    <div className="space-y-4">
      ...
      {mode === "cards" ? <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">{visible.map((item) => {
        ...
        return <article key={item.queueId} className="min-w-0 rounded-3xl border border-slate-200 bg-white p-4 shadow-sm">
          ...
          <dl className="mt-4 grid grid-cols-2 gap-2 text-xs"><div><dt className="text-slate-500">시간대</dt><dd className="font-bold">{item.registeredDate} {item.slot}</dd></div><div><dt className="text-slate-500">ASR</dt><dd className="font-bold">{item.asrScore ?? "-"}</dd></div><div><dt className="text-slate-500">사람검토</dt><dd className="font-bold">{item.humanReview || "미검토"}</dd></div><div><dt className="text-slate-500">오류</dt><dd className="line-clamp-2 font-bold text-red-700">{item.errorMemo || "없음"}</dd></div></dl>

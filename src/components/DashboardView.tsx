    ...
    <div className="space-y-6">
      ...
      <section className="rounded-lg border border-slate-200 bg-white p-5 shadow-sm">
        <div className="flex flex-col gap-3 lg:flex-row lg:items-start lg:justify-between">
          <div>
            <h2 className="text-base font-bold text-slate-950">Candidate Seed Plan</h2>
            <p className="mt-1 text-sm text-slate-600">
              Read-only candidate seed dry-run summary. Dashboard does not run collectors, create queue rows, create worker jobs, or upload.
            </p>
          </div>
          <a href="/candidates/analytics#seed-plan" className="rounded-md border border-slate-200 px-3 py-2 text-sm font-bold text-slate-700 hover:bg-slate-100">
            Open seed planner
          </a>
        </div>
        <div className="mt-4 grid gap-3 sm:grid-cols-4">
          <InfoPill label="Strategy" value={candidateSeedPlanSummary?.strategy ?? "balanced"} />
          <InfoPill label="Recommended keywords" value={String(candidateSeedPlanSummary?.keyword_count ?? 0)} />
          <InfoPill label="Estimated candidates" value={String(candidateSeedPlanSummary?.estimated_candidate_limit ?? 0)} />
          <InfoPill label="Safety" value={`collector_executed=${String(candidateSeedPlanSummary?.collector_executed ?? false)}`} />
        </div>
      </section>

...
export type ScheduledPrivatePilotApproval = {
  decision: "PASS";
  package_id: string;
  approved_at: string;
  expires_at: string;
...
export async function executeScheduledPrivatePilot(input: {
  ...
  approval?: ScheduledPrivatePilotApproval;
  adapter: YouTubeUploadAdapter;
  now?: string | Date;
  dailyPrivateUploadCount: number;
  duplicateUploadExists: boolean;
  safety: {
  ...
  if (!result.succeeded || !result.side_effects.uploaded) {
    return result.side_effects.external_api_called
      ? humanReview("YOUTUBE_UPLOAD_EXTERNAL_CALL_FAILED", 1)
      : blocked(result.blocked_reasons[0] ?? "YOUTUBE_UPLOAD_BLOCKED", 1);
  }
...
function validate(input: Parameters<typeof executeScheduledPrivatePilot>[0]) {
  ...
  if (input.uploadPackage.id !== input.approval?.package_id || input.approval?.decision !== "PASS") {
    return "FRESH_OWNER_APPROVAL_REQUIRED";
  }
  if (!input.approval.nonce.trim()) return "FRESH_OWNER_APPROVAL_REQUIRED";
  ...
  if (input.dailyPrivateUploadCount >= 1) return "DAILY_PRIVATE_UPLOAD_LIMIT_REACHED";
  if (input.duplicateUploadExists || input.uploadPackage.uploaded_at || input.uploadPackage.uploaded_url) {

...
export function listCommerceEventsForWindow(
  ...
  const events = [
    ...yearsInWindow(window).flatMap(staticEventsForYear),
...
function staticEventsForYear(year: number): CommerceEvent[] {
  ...
  return [
    ...
    ...(lunarNewYear
      ? [rangeEvent("seollal", "설날 연휴·선물 준비", addDays(lunarNewYear, -1), addDays(lunarNewYear, 1), "holiday", "static_calendar")]
      : []),
...
function buildSambokEvents(year: number): CommerceEvent[] {
  return [
    fixedEvent("chobok", "초복", nthGengDayOnOrAfter(`${year}-06-21`, 3), "season", "seasonal_rule"),
    fixedEvent("jungbok", "중복", nthGengDayOnOrAfter(`${year}-06-21`, 4), "season", "seasonal_rule"),
    fixedEvent("malbok", "말복", nthGengDayOnOrAfter(`${year}-08-07`, 1), "season", "seasonal_rule")
  ];
}
...
function intersectsWindow(event: CommerceEvent, window: EventWindow) {
  return eventStart(event) <= window.endDate && eventEnd(event) >= window.startDate;
}
...
function findDangiDate(relatedYear: number, lunarMonth: number, lunarDay: number) {
  ...
  while (date <= end) {
    ...
    if (year === relatedYear && month === lunarMonth && day === lunarDay) return formatDateOnly(date);
...
function nthGengDayOnOrAfter(startDate: string, occurrence: number) {
  ...
  for (let offset = 0; offset < 60; offset += 1) {
    if (mod(daysBetween(GENG_DAY_REFERENCE, date), 10) === 0 && ++found === occurrence) return date;
    date = addDays(date, 1);
  }
  return startDate;
}

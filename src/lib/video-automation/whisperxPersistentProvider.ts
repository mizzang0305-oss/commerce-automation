...
export type WhisperXResponse = {
  ...
  transcript_source?: "provided_local_asr" | "provided_narration_intent" | "whisperx_transcribe";
};
...
export class PersistentWhisperXProvider {
  ...
  async align(audioPath: string, transcript?: string, transcriptRole: "local_asr" | "narration_intent" = "local_asr"): Promise<WhisperXResponse> {
    await this.start();
    ...
    return new Promise<WhisperXResponse>((resolve, reject) => {
      ...
      this.child?.stdin.write(`${JSON.stringify({ id, audio_path: audioPath, transcript: transcript?.trim() || undefined, transcript_role: transcriptRole, language: "ko", model: "tiny", device: "cpu", compute_type: "int8" })}\n`);
    });
  }

...
export type Level3Gate = {
  ...
  actual: string;
  reason: string;
  severity: "completion" | "integrity" | "telemetry";
};

export type Level3CompletionMatrix = {
  schemaVersion: "daily69-level3-completion-matrix-v1";
  completion: Level3Completion;
  terminalState: "CLEAN_COMPLETE" | "COMPLETE_WITH_BLOCKED_ITEMS" | "PENDING" | "FAILED";
  perfectDay: boolean;
  pass: number;
  unproven: number;
  failed: number;
...
export function validateLevel3Completion(input: Level3CompletionInput): Level3CompletionMatrix {
  ...
  completion("queue_outcomes", input.queue.ready + input.queue.blocked === expected.total && input.queue.failed === 0,
    `ready+blocked=${expected.total} failed=0`, `${input.queue.ready}+${input.queue.blocked}/${input.queue.failed}`, "DAILY69_QUEUE_OUTCOME_COUNT_INCOMPLETE");
  ...
  integrity("queue_failures", input.queue.failed === 0, "failed=0; blocked allowed", `${input.queue.failed}; blocked=${input.queue.blocked}`, "DAILY69_QUEUE_FATAL_FAILURE_PRESENT");
  ...
  // Initial active/reserve admission remains fail-closed before ARM. At closeout,
  // however, reserve consumption and exhausted slot-local candidates are valid
  // telemetry and must not prevent an otherwise terminal day from closing.
  gates.push(gate("capacity", "PASS", "operational fallback coverage telemetry only", capacityActual,
    "DAILY69_OPERATIONAL_CAPACITY_TELEMETRY", "telemetry"));
  ...
  completion("codex_queue_reviews", input.queue.codexReviews === input.queue.ready, String(input.queue.ready), input.queue.codexReviews, "DAILY69_CODEX_REVIEW_INCOMPLETE");
  ...
  const expectedDirectReady = input.queue.ready - expected.prevalidatedReady;
  completion("review_evidence_modes", expectedDirectReady >= 0 && input.queue.directReviewBindings === expectedDirectReady
    && input.queue.immutableCarryForwardBindings === expected.prevalidatedReady,
  `direct/carry=${expectedDirectReady}/${expected.prevalidatedReady}`,
  ...
  if (!retained) {
    ...
    completion("media_qa_review", retained.media.validVideoArtifacts === input.queue.ready && retained.media.missingVideoArtifacts === input.queue.blocked
      && retained.media.machineQaPassed === input.queue.ready && retained.media.finalQaPassed === input.queue.ready
      && retained.media.codexReviewBindings === input.queue.ready && retained.media.exactVideoHashBindings === input.queue.ready
      && retained.media.directReviewBindings === expectedDirectReady
      && retained.media.immutableCarryForwardBindings === expected.prevalidatedReady,
    `valid/missing/invalid/machine/final/codex/hash/direct/carry=${input.queue.ready}/${input.queue.blocked}/0/${input.queue.ready}/${input.queue.ready}/${input.queue.ready}/${input.queue.ready}/${expectedDirectReady}/${expected.prevalidatedReady}`,
    ...
    integrity("run_integrity", retained.runs.failed === 0 && retained.runs.retried === 0 && retained.runs.runIdsMatched === true
    ...
    "failed/retried=0/0 runIdsMatched=true batchClaimResultCardinalityMatched=true duplicateClaim/duplicateResult=0/0",
    `${retained.runs.failed}/${retained.runs.retried}/${retained.runs.runIdsMatched}/${retained.runs.batchClaimResultCardinalityMatched}/${retained.runs.duplicateClaimIds}/${retained.runs.duplicateResultIds}`,
  ...
  if (input.executionMode === "functional_shadow") {
    const runs = input.retainedEvidence?.runs;
    completion("functional_execution", Boolean(runs)
      && runs!.scheduledBatchRuns === expected.scheduledBatchRuns
      && runs!.batchResults === expected.scheduledBatchRuns
      && runs!.claimed === expected.scheduledRemaining
      && runs!.completed + runs!.blocked === expected.scheduledRemaining
      && runs!.failed === 0 && runs!.retried === 0,
    `functional runs/results/claimed/terminal=${expected.scheduledBatchRuns}/${expected.scheduledBatchRuns}/${expected.scheduledRemaining}/${expected.scheduledRemaining}`,
    runs ? `${runs.scheduledBatchRuns}/${runs.batchResults}/${runs.claimed}/${runs.completed + runs.blocked}` : "missing",
    "DAILY69_FUNCTIONAL_EXECUTION_UNPROVEN");
  } else if (!natural) {
  ...
  const terminalState = result === "FAILED" ? "FAILED" : result === "PENDING" ? "PENDING"
    : input.queue.blocked > 0 ? "COMPLETE_WITH_BLOCKED_ITEMS" : "CLEAN_COMPLETE";
  return {
    schemaVersion: "daily69-level3-completion-matrix-v1",
    completion: result,
    terminalState,
    perfectDay: result === "PASS" && input.queue.ready === expected.total && input.queue.blocked === 0,

...
export async function POST(request: Request, context: { params: Promise<{ id: string }> }) {
  ...
  const errorMessage = typeof body.error_message === "string" ? body.error_message : "Worker job failed.";
  if (!workerId) {
    return NextResponse.json({ ok: false, message: "worker_id is required." }, { status: 400 });
  }

  const job = await getAutomationRepository().failWorkerJob(id, workerId, errorMessage);
  if (!job) {

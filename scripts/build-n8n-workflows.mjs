...
const nightlyPrepareCode = [
  ...
  "if (!$vars.COUPANG_ACCESS_KEY || !$vars.COUPANG_SECRET_KEY) return safeFailure('쿠팡파트너스 API 설정이 없어 큐를 생성할 수 없습니다.');",
  ...
  "  const signature = crypto.createHmac('sha256', $vars.COUPANG_SECRET_KEY).update(message).digest('hex');",
  "  return { url: 'https://api-gateway.coupang.com' + path + query, authorization: 'CEA algorithm=HmacSHA256, access-key=' + $vars.COUPANG_ACCESS_KEY + ', signed-date=' + signedDate + ', signature=' + signature };",
  "}",

...
export async function generateV016ReviewPacket(input = {}) {
  ...
  await writeJson(readinessPath, voiceProvider);
...
export function buildV016ReviewSummary(input) {
  ...
  const recognizedCoreAnchors = Array.isArray(asrProbe.recognized_core_anchors)
    ? asrProbe.recognized_core_anchors.filter((value) => typeof value === "string")
    : [];

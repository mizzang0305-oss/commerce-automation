...
try {
    ...
    & npm.cmd run daily69:first-day:preflight --silent -- --runtime --sheets
    if ($LASTEXITCODE -ne 0) { throw "FIRST_OPERATION_PREFLIGHT_FAILED" }
    $started = Get-Date
    $output = & npm.cmd run queue-video:run-next --silent 2>&1
    $queueExit = $LASTEXITCODE
    $stamp = Get-Date -Format "yyyyMMddHHmmss"
    $output | Set-Content -LiteralPath (Join-Path $QueueRoot "batch-results\batch-$stamp.jsonl") -Encoding utf8
    $output | Write-Output
    & npm.cmd run queue-control:project --silent
    if ($queueExit -ne 0) { exit $queueExit }
    exit $LASTEXITCODE
} catch {
    $reason = if ($_.Exception.Message -match '^[A-Z0-9_:-]+$') { $_.Exception.Message } else { "FIRST_OPERATION_BATCH_FAILED" }

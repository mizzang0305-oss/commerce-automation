...
async function main() {
  const cwd = process.env.V108_CWD ||
    process.env.V107_CWD ||
    process.env.V106_CWD ||
    process.env.V095_CWD ||
    process.cwd();
  const env = loadLocalEnv(cwd, process.env);
  const report = await buildV108FirstVideoUploadPackageMaterializerReport({
    cwd,
    env,
    selectedChannelKey: env.V108_CHANNEL_KEY || env.V107_CHANNEL_KEY || env.V106_CHANNEL_KEY || "father_jobs",
    mode: env.V108_MODE || "dry_run",
...
if (process.argv[1] && import.meta.url === pathToFileURL(process.argv[1]).href) {
  main().catch(() => {
    process.stdout.write(`${JSON.stringify({
      ...
      FINAL_STATUS: "BLOCKED_V108_UPLOAD_PACKAGE_STILL_MISSING_NO_UPLOAD",

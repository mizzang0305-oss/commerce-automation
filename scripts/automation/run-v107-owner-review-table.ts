...
async function main() {
  const cwd = process.env.V107_CWD || process.env.V106_CWD || process.env.V095_CWD || process.cwd();
  const env = loadLocalEnv(cwd, process.env);
  const report = await buildV107OwnerReviewFirstVideoSettingsTable({
    cwd,
    env,
    selectedChannelKey: env.V107_CHANNEL_KEY || env.V106_CHANNEL_KEY || env.V105_CHANNEL_KEY || "father_jobs",
    mode: env.V107_MODE || "dry_run",

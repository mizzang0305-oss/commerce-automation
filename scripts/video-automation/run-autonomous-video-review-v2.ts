...
async function main(): Promise<void> {
  ...
  const outputRoot = resolveVideoOutputRoot(runId);
...
async function loadLiveProductInputs(path: string, runId: string): Promise<ReturnType<typeof loadApprovedProductFixtures>> {
  ...
  if (!Array.isArray(value.products) || value.products.length < 1 || value.products.length > 3) throw new Error("LIVE_PRODUCT_VIDEO_INPUT_COUNT_INVALID");

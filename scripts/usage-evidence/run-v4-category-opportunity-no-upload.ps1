...
param(
    ...
    [ValidateRange(0, 30)]
    [int]$MaxProviderCalls = 20,

    [Parameter(Mandatory = $false)]
    [ValidateRange(1, 240)]
    [int]$MaxRawDiscoveries = 240
)
...
try {
    ...
    $locationPushed = $true
    & npx.cmd tsx scripts/usage-evidence/run-v4-category-opportunity.ts `
        ...
        --max-raw-discoveries $MaxRawDiscoveries

...
One-product real product video asset planning is approval-gated at `POST /api/uploads/youtube/real-product-pilot/video-asset/prepare` and in the `/uploads` dashboard. It validates a single real candidate, blocks smoke/test candidates, keeps local video generation local-only, validates server-accessible `video/mp4` asset references, and returns a one-row `product_assets` write plan without executing R2 upload, Supabase writes, queue/job creation, upload packages, or YouTube Execute.

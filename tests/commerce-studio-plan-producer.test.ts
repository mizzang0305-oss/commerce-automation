...
const generationSlots = ["09:00", "15:00", "21:00"];
const roots: string[] = [];
...
describe("Commerce Studio isolated owner → host → producer flow", () => {
  it("rejects a same-day past slot, invalid calendar day and unscheduled slot", async () => {
    const store = new InMemorySimpleProducerStore();
    const now = new Date("2026-09-24T01:00:00.000Z");
    const command = { ...select(), type: "HOLD_PLAN" as const, payload: {}, expiresAt: "2026-09-24T02:00:00.000Z" };
    expect((await applyStudioHostCommand({ command: { ...command, targetId: "2026-09-24|09:00" },
      producerStore: store, ...host, generationSlots, now })).safeError).toBe("STUDIO_PLAN_SLOT_PAST");
    expect((await applyStudioHostCommand({ command: { ...command, commandId: randomUUID(), targetId: "2026-02-30|09:00" },
      producerStore: store, ...host, generationSlots, now })).safeError).toBe("STUDIO_PLAN_TARGET_INVALID");
    expect((await applyStudioHostCommand({ command: { ...command, commandId: randomUUID(), targetId: "2026-09-25|10:00" },
      producerStore: store, ...host, generationSlots, now })).safeError).toBe("STUDIO_PLAN_SLOT_NOT_SCHEDULED");
  });

  it("rejects a product reserved by another future plan", async () => {
    const store = new InMemorySimpleProducerStore({ schema: "simple-producer/v1", slots: [], studioCandidates: [
      { snapshotId: "snapshot-B", slotId, sourceRevision: "fixture-1", productId: productB,
        productName: "B", channelKey: "father_jobs", eligible: true,
        eligibilityCheckedAt: "2026-09-23T00:00:00.000Z", safeBlockers: [] }
    ], studioPlans: [{ planId: "2026-09-24|15:00", date: "2026-09-24", slot: "15:00", version: 1,
      status: "selected", selectionMode: "manual", candidateSnapshotId: "another", exactProductId: productB,
      channelKey: "father_jobs", selectedAt: "2026-09-23T00:00:00.000Z", lockedAt: null, executionRecordId: null }] });
    const receipt = await applyStudioHostCommand({ command: select(), producerStore: store, ...host,
      generationSlots, now: new Date("2026-09-23T01:00:00.000Z") });
    expect(receipt.safeError).toBe("STUDIO_CANDIDATE_NOT_ELIGIBLE");
  });
  ...
  it("selects exact B, retains pending until ACK and passes exact B into producer input", async () => {
    ...
    const store = new InMemorySimpleProducerStore({ schema: "simple-producer/v1", slots: [], studioCandidates: [
      { snapshotId: "snapshot-B", slotId, sourceRevision: "fixture-1", productId: productB, productName: "격리 상품 B", channelKey: "neoman_moleulgeol", eligible: true, eligibilityCheckedAt: "2026-09-23T00:59:00.000Z", safeBlockers: [] }
    ] });
    const command = select();
    expect((await bridge.enqueue(command, host.ownerId)).status).toBe("pending");
    const receipt = await applyStudioHostCommand({ command, producerStore: store, ...host, generationSlots, now: new Date("2026-09-23T01:00:00.000Z") });
    ...
    expect(await applyStudioHostCommand({ command, producerStore: store, ...host, generationSlots, now: new Date("2026-09-23T01:01:00.000Z") })).toEqual(receipt);
  ...
  it("fails closed on stale version, ineligible candidate, post-claim edit and wrong pipeline product", async () => {
    const store = new InMemorySimpleProducerStore({ schema: "simple-producer/v1", slots: [], studioCandidates: [
      { snapshotId: "snapshot-B", slotId, sourceRevision: "fixture-1", productId: productB, productName: "B", channelKey: "father_jobs", eligible: true, eligibilityCheckedAt: "2026-09-23T00:59:00.000Z", safeBlockers: [] }
    ] });
    const now = new Date("2026-09-23T01:00:00.000Z");
    expect((await applyStudioHostCommand({ command: select(productC), producerStore: store, ...host, generationSlots, now })).safeError).toBe("STUDIO_CANDIDATE_NOT_ELIGIBLE");
    const chosen = select();
    expect((await applyStudioHostCommand({ command: chosen, producerStore: store, ...host, generationSlots, now })).status).toBe("applied");
    expect((await applyStudioHostCommand({ command: select(productB, 0), producerStore: store, ...host, generationSlots, now })).safeError).toBe("STUDIO_PLAN_VERSION_STALE");
    const root = await mkdtemp(join(tmpdir(), "studio-plan-")); roots.push(root);
    const publisherStore = new InMemoryYouTubePublicPublisherStore({ jobs: [], ledger: [] });
    const result = await runSimpleProducerOnce({
    ...
    expect((await applyStudioHostCommand({ command: afterClaim, producerStore: store, ...host, generationSlots, now: new Date("2026-09-24T01:00:00.000Z") })).safeError).toBe("STUDIO_PLAN_SLOT_PAST");
  });
  ...
  it("does not mark settings applied until both config and Task readback match", async () => {
    ...
    const context = { command, producerStore: store, settingsAdapter: adapter, ...host, generationSlots, now: new Date("2026-09-23T01:00:00.000Z") };
    expect((await applyStudioHostCommand(context)).status).toBe("pending");
    expect(config.revision).toBe(1);
    expect(configWrites).toBe(1);
    expect((await store.read()).studioSettingsRevision ?? 0).toBe(0);
    failTask = false;
    ...
    expect((await store.read()).studioSettingsRevision).toBe(1);
  });

  it("does not claim a generation slot when the config and committed settings revision diverge", async () => {
    const store = new InMemorySimpleProducerStore({ schema: "simple-producer/v1", slots: [], studioSettingsRevision: 1 });
    const executePipeline = vi.fn();
    const result = await runSimpleProducerOnce({
      config: { schema: "simple-producer/v1", enabled: true, dailyGenerateTarget: 3, maxItemsPerRun: 1,
        generationSlots: ["09:00"], timeZone: "Asia/Seoul", evidenceRoot: "D:\\isolated", studioSettingsRevision: 0 },
      producerStore: store, publisherStore: new InMemoryYouTubePublicPublisherStore({ jobs: [], ledger: [] }),
      now: new Date("2026-09-24T00:00:00.000Z"), executePipeline
    });
    expect(result).toMatchObject({ status: "settings_reconcile_required", readyJobCreated: 0, videosInsertCalls: 0 });
    expect(executePipeline).not.toHaveBeenCalled();
    expect((await store.read()).slots).toHaveLength(0);
  });
});

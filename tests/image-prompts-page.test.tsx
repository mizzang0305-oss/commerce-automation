...
describe("/image-prompts page", () => {
  test("renders copy-only image planning UI with no generation or upload actions", async () => {
    ...
    expect(screen.getAllByText("ready_for_slideshow_plan=false").length).toBeGreaterThan(0);
    expect(screen.getAllByText("local_file_read=false").length).toBeGreaterThan(0);
    expect(screen.getAllByText("r2_uploaded=false").length).toBeGreaterThan(0);
    expect(screen.getByRole("heading", { name: "Selected Image Slideshow Package Plan" })).toBeInTheDocument();
    expect(screen.getByText("15-second slideshow timeline")).toBeInTheDocument();
    expect(screen.getByText("FFmpeg command preview")).toBeInTheDocument();
    expect(screen.getByText("MoviePy script preview")).toBeInTheDocument();
    expect(screen.getByText("manual render checklist")).toBeInTheDocument();
    expect(screen.getByRole("button", { name: "Preview slideshow package plan" })).toBeInTheDocument();
    expect(screen.getByRole("button", { name: "Copy slideshow package JSON" })).toBeInTheDocument();
    expect(screen.getByRole("button", { name: "Copy timeline markdown" })).toBeInTheDocument();
    expect(screen.getByRole("button", { name: "Copy FFmpeg command preview" })).toBeInTheDocument();
    expect(screen.getByRole("button", { name: "Copy MoviePy script preview" })).toBeInTheDocument();

...
describe("real product video asset entrypoint UI", () => {
  it("renders approval-gated video asset controls without upload or worker execution actions", () => {
    ...
    expect(screen.getByRole("button", { name: /Generate local-only video contract/i })).toBeInTheDocument();
    expect(screen.getByRole("button", { name: /Register server asset contract/i })).toBeInTheDocument();

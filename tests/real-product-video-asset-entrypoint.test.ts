...
describe("one-product video asset entrypoint", () => {
  ...
  it("accepts the one-shot story voiceover approval for local generation", async () => {
    const result = await buildOneProductVideoAssetEntryPoint({
      ...
      localVideoGenerator: async (item) => ({
        candidate_id: item.id,
        local_video_path: "C:\\Users\\LOVE\\MyProjects\\commerce-automation\\commerce-assets\\output\\video-packages\\real-product-candidate-real-asset-001\\candidate-real-asset-001_story_voiceover_pacing_v002.mp4",
        mime_type: "video/mp4",
        size_bytes: 8192,
        duration_seconds: 25,
        checksum_sha256: "d".repeat(64),

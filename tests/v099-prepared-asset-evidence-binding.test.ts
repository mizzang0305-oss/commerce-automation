...
import { validatePreparedVideoAssetRef } from "../src/lib/uploads/assets/preparedVideoAssetValidator";
...
const CHANNEL_IDS: Record<ChannelKey, string> = {
  father_jobs: FULL_CHANNEL_ID,
  neoman_moleulgeol: `UC${"7".repeat(22)}`,
  lets_buy: `UC${"8".repeat(22)}`
};
...
describe("v099 prepared asset evidence binding no-upload", () => {
  ...
  test.each<ChannelKey>(["father_jobs", "neoman_moleulgeol", "lets_buy"])(
    "uses selectedChannelKey from context when binding prepared evidence for %s",
    async (channelKey) => {
      const cwd = await writeContextBackedCwd(channelKey);
      try {
        const report = await buildV099PreparedAssetEvidenceDryRun({
          cwd,
          env: readyEnv(),
          loadUploadPackages: async () => [readyUploadPackageFor(channelKey)],
          preparedVideoAssetRefs: {
            [channelKey]: preparedVideoAssetRef({
              provider: "signed_https",
              checksum_sha256: `${channelKey}-video-hash`,
              signed_url: `https://asset-bridge.example.test/private/${channelKey}.mp4?signature=secret`,
              prepared_video_asset_url: null
            })
          }
        });

        expect(report.selectedChannelKey).toBe(channelKey);
        expect(report.status).toBe("prepared_asset_evidence_ready");
        expect(report.preparedAssetEvidencePresent).toBe(true);
        expect(report.preparedAssetProviderLabel).toBe("signed_https");
        expect(report.preparedAssetHashPrefix).toBe(`${channelKey}-video-hash`.slice(0, 10));
        expect(report.uploadRequestBuilt).toBe(true);
        expect(report.videosInsertCalled).toBe(false);
        expect(report.commentThreadsInsertCalled).toBe(false);
        expect(JSON.stringify(report)).not.toMatch(FORBIDDEN_REPORT_PATTERN);
  ...
  test("does not become ready from a prepared ref for a different channel", async () => {
    const cwd = await writeContextBackedCwd("lets_buy");
    try {
      const report = await buildV099PreparedAssetEvidenceDryRun({
        cwd,
        env: readyEnv(),
        loadUploadPackages: async () => [readyUploadPackageFor("lets_buy")],
        preparedVideoAssetRefs: {
          father_jobs: preparedVideoAssetRef({
            provider: "external_https",
            prepared_video_asset_url: RAW_PREPARED_URL
      ...
      expect(report.selectedChannelKey).toBe("lets_buy");
      expect(report.status).toBe("blocked");
      expect(report.preparedAssetEvidencePresent).toBe(false);
      expect(report.uploadRequestBuilt).toBe(false);
      expect(report.resolverBlocker).toBe("BLOCKED_V081_VIDEO_ASSET_MISSING");
  ...
  test.each(["signed_https", "r2_signed_url", "supabase_signed_url"] as const)(
    "prepared asset validator accepts %s provider with HTTPS evidence",
    (provider) => {
      const result = validatePreparedVideoAssetRef(preparedVideoAssetRef({
        provider,
        size_bytes: 1024,
        signed_url: RAW_SIGNED_URL,
        prepared_video_asset_url: null
      ...
      expect(result.asset_ref?.provider).toBe(provider);
      expect(result.blocked_reasons).toEqual([]);
      expect(JSON.stringify(result.safe_display)).not.toMatch(FORBIDDEN_REPORT_PATTERN);
    }
  );

  test("prepared asset validator still blocks storage-key-only evidence for uploadability", () => {
    const result = bindV099PreparedVideoAssetEvidence({
      preparedVideoAssetRef: preparedVideoAssetRef({
        provider: "r2_signed_url",
        storage_key: "private/v099.mp4",
        signed_url: null,
        prepared_video_asset_url: null
      }),
    ...
    expect(result.preparedAssetEvidencePresent).toBe(true);
    expect(result.preparedAssetUploadableUrlPresent).toBe(false);
...
function readyEnv(overrides: NodeJS.ProcessEnv = {}): NodeJS.ProcessEnv {
  return {
    ...
    YOUTUBE_FATHER_JOBS_CHANNEL_ID: CHANNEL_IDS.father_jobs,
    YOUTUBE_NEOMAN_MOLEULGEOL_CHANNEL_ID: CHANNEL_IDS.neoman_moleulgeol,
    YOUTUBE_LETS_BUY_CHANNEL_ID: CHANNEL_IDS.lets_buy,
    ...overrides
  };
}

async function writeContextBackedCwd(channelKey: ChannelKey = "father_jobs") {
  ...
  await writeFile(contextPath, `${JSON.stringify({
    ...
    videoAssetHashPrefix: ids.videoHashPrefix,
    targetChannelHashPrefix: hashPrefix(CHANNEL_IDS[channelKey]),
...
function readyUploadPackage(overrides: Partial<V073UploadPackage> = {}): V073UploadPackage & {
  ...
  return {
    ...readyUploadPackageFor("father_jobs"),
    ...overrides
  };
}

function readyUploadPackageFor(channelKey: ChannelKey): V073UploadPackage & {
  shortsContentQuality: unknown;
} {
  const ids = idsFor(channelKey);
  const base: V073UploadPackage & { shortsContentQuality: unknown } = {
    ...
    queueItemId: ids.queueItemId,
    generatedContentId: ids.generatedContentId,
    channelKey,
    ...
    videoAsset: {
      ...
      hashEvidence: ids.videoHashPrefix,
    ...
    targetChannel: {
      channelKey,
      channelIdHashPrefix: hashPrefix(CHANNEL_IDS[channelKey]),
      formatValid: true,
...
function idsFor(channelKey: ChannelKey) {
  return {
    packageId: `pkg-v099-${channelKey}`,
    queueItemId: `queue-v099-${channelKey}`,
    generatedContentId: `generated-v099-${channelKey}`,
    videoHashPrefix: `${channelKey}-video`
  };
}

...
describe("first no-upload Daily69 operation", () => {
  ...
  it("derives carry-forward, remaining, and schedule counts from an exact 12-ready source", async () => {
    ...
    const armed = await armFirstOperation({ sourceRoot: fixture.sourceRoot, operationBase: fixture.operationBase, assetBoundaryRoot: fixture.parent, now: new Date("2026-08-09T17:00:00.000Z"), expectedGitHead: "9".repeat(40) });
    const snapshot = await firstOperationStatus(armed.operationRoot);
  ...
  it("fails closeout before lifecycle mutation when the promoted pointer is inconsistent", async () => {
    const fixture = await sourceFixture();
    const armed = await armFirstOperation({ sourceRoot: fixture.sourceRoot, operationBase: fixture.operationBase, assetBoundaryRoot: fixture.parent, now: new Date("2026-08-09T17:00:00.000Z"), expectedGitHead: "8".repeat(40) });
    await transitionFirstOperationArmStatus(armed.operationRoot, "projection_verified");
    await transitionFirstOperationArmStatus(armed.operationRoot, "tasks_armed");
...
async function sourceFixture(options: { missingAffiliateRanks?: number[]; readyCount?: number } = {}) {

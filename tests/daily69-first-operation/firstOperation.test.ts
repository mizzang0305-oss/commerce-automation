...
describe("first no-upload Daily69 operation", () => {
  it("carries the prevalidated nine without rerendering and schedules exactly 20 hourly groups of three", async () => {
    ...
    const armed = await armFirstOperation({ sourceRoot: fixture.sourceRoot, operationBase: fixture.operationBase, usageMaterializationAssetRoot: fixture.sourceRoot, now: new Date("2026-08-09T17:00:00.000Z"), expectedGitHead: "a".repeat(40) });
  ...
  it("is idempotent and fails closed when original evidence changes", async () => {
    const fixture = await sourceFixture();
    const input = { sourceRoot: fixture.sourceRoot, operationBase: fixture.operationBase, assetBoundaryRoot: fixture.parent, usageMaterializationAssetRoot: fixture.sourceRoot, now: new Date("2026-08-09T17:00:00.000Z"), expectedGitHead: "b".repeat(40) };
  ...
  it("preserves an explicit repaired-video origin and regeneration count across the operation clone", async () => {
    ...
    const armed = await armFirstOperation({ sourceRoot: fixture.sourceRoot, operationBase: fixture.operationBase, assetBoundaryRoot: fixture.parent, usageMaterializationAssetRoot: fixture.sourceRoot, now: new Date("2026-08-09T17:00:00.000Z"), expectedGitHead: "1".repeat(40) });
    const snapshot = await firstOperationStatus(armed.operationRoot);
  ...
  it("derives carry-forward, remaining, and schedule counts from an exact 12-ready source", async () => {
    ...
    const armed = await armFirstOperation({ sourceRoot: fixture.sourceRoot, operationBase: fixture.operationBase, assetBoundaryRoot: fixture.parent, usageMaterializationAssetRoot: fixture.sourceRoot, now: new Date("2026-08-09T17:00:00.000Z"), expectedGitHead: "9".repeat(40) });
    const snapshot = await firstOperationStatus(armed.operationRoot);
  ...
  it("promotes the active pointer only after projection and task verification", async () => {
    const fixture = await sourceFixture();
    const armed = await armFirstOperation({ sourceRoot: fixture.sourceRoot, operationBase: fixture.operationBase, assetBoundaryRoot: fixture.parent, usageMaterializationAssetRoot: fixture.sourceRoot, now: new Date("2026-08-09T17:00:00.000Z"), expectedGitHead: "e".repeat(40) });
  ...
  it("fails closeout before lifecycle mutation when the promoted pointer is inconsistent", async () => {
    const fixture = await sourceFixture();
    const armed = await armFirstOperation({ sourceRoot: fixture.sourceRoot, operationBase: fixture.operationBase, assetBoundaryRoot: fixture.parent, usageMaterializationAssetRoot: fixture.sourceRoot, now: new Date("2026-08-09T17:00:00.000Z"), expectedGitHead: "8".repeat(40) });
    await transitionFirstOperationArmStatus(armed.operationRoot, "projection_verified");
    await transitionFirstOperationArmStatus(armed.operationRoot, "tasks_armed");
  ...
  it("rejects a 68/69 affiliate-ready source before creating an operation or active pointer", async () => {
    ...
    await expect(armFirstOperation({ sourceRoot: fixture.sourceRoot, operationBase: fixture.operationBase, assetBoundaryRoot: fixture.parent, usageMaterializationAssetRoot: fixture.sourceRoot, now: new Date("2026-08-09T17:00:00.000Z"), expectedGitHead: "d".repeat(40) }))
...
async function sourceFixture(options: { missingAffiliateRanks?: number[]; readyCount?: number; incompatibleAllocation?: "active" | "reserve" } = {}) {
  ...
  const registry = makeUsageEvidenceRegistry({ packsPerUseCase: 6 });
  ...
  for (const asset of registry.assets) {
    ...
    const path = join(sourceRoot, asset.sourceRelativeReference);
